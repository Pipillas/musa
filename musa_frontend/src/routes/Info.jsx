...
function Info() {
    ...
    const fetchProductos = (page, search, isCarrito, isFavorito) => socket.emit('request-productos', { page, search, isCarrito, isFavorito });
    ...
    useEffect(() => {
        socket.on('cambios', () => {
            ...
            fetchProductos(page, search, isCarrito, isFavorito);
        });
    ...
    }, [page, search, producto, showModal, isCarrito, isFavorito]);
        ...
        <div className='tabla-container-info'>
            <div className="buscador">
                ...
                <div className="buscar-favorito">
                    <button className={`toggle-button ${isFavorito ? 'active' : ''}`} onClick={toggleFavorito}>
                        <i className="bi bi-heart-fill"></i>
                    </button>
                    <button className={`toggle-carrito toggle-button ${isCarrito ? 'active' : ''}`} onClick={toggleCarrito}>
                    ...
                    <tbody>
                        {productos?.map((producto) => (
                            <tr className="tr-info" key={producto._id} >
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.codigo}</td>
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.nombre}</td>
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.year}</td>
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.bodega}</td>
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.cepa}</td>
                                <td onClick={() => productoClickeado(producto.codigo)}>{producto.origen}</td>
                                ...
                                <td onClick={() => agregarCarrito(producto._id)} className="carrito"><span><i className="bi bi-cart4"></i></span></td>
                                <td onClick={() => agregarFavorito(producto._id)} className="td-fav"><span><i className="bi bi-heart-fill"></i></span></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                    ...
                    <div className="modal">
                        <div className="modal-body">
                            ...
                            <div className="modal-info">
                                ...
                                <div className='cepa'>
                                    <span>{producto.cepa} de {producto.origen}</span>
                                </div>
                                ...
                                <div className='modal-precios'>
                                    <div className="modal-precios-stock">
                                        <span><i className="bi bi-tags"></i> <NumericFormat prefix='$' displayType='text' value={producto.venta} thousandSeparator="." decimalSeparator=',' /></span>
